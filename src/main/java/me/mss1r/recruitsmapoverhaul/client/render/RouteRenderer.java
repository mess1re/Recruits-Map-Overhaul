...
public class RouteRenderer {
    ...
    private static final int COLOR_NORMAL      = 0xFFF4F9FF;
    private static final int COLOR_NOT_LOADED  = 0xFFFF7A7A;
    private static final int COLOR_LINE        = 0xCC7FDBFF;
    private static final int COLOR_INSERT      = 0xFF5BE7FF;
    ...
    private static void renderLines(GuiGraphics guiGraphics, List<Waypoint> waypoints,
        ...
        for (int i = 0; i < waypoints.size() - 1; i++) {
            ...
            drawLine(guiGraphics, x1, z1, x2, z2, COLOR_LINE);
        }
    }
    ...
    private static void renderWaypointIcon(GuiGraphics guiGraphics, Waypoint waypoint, int number,
        ...
        guiGraphics.pose().translate(pixelX, pixelZ, 0);
        guiGraphics.fill(-textWidth / 2 - 2, -11, textWidth / 2 + 3, -2, 0x900D131B);
        guiGraphics.drawString(Minecraft.getInstance().font, numStr,
                -textWidth / 2, -10, argb, false);
        ...
        if (scale > 2.0 && !isDragging) {
            ...
            if (label != null) {
                ...
                guiGraphics.pose().pushPose();
                guiGraphics.pose().translate(pixelX, pixelZ, 0);
                guiGraphics.fill(-labelW / 2 - 2, 4, labelW / 2 + 3, 13, 0x900D131B);
                guiGraphics.drawString(Minecraft.getInstance().font, label,
                        -labelW / 2, 5, argb, false);
                guiGraphics.pose().popPose();
            }
        }
    }
    ...
    private static void drawLine(GuiGraphics g, double x1, double y1, double x2, double y2, int color) {
        MapRenderUtil.line(g, x1, y1, x2, y2, 1.35, color);
    }
    ...
    public static Waypoint getWaypointAt(RecruitsRoute route, double mouseX, double mouseY,
        ...
        for (Waypoint wp : route.getWaypoints()) {
            double px = offsetX + wp.getPosition().getX() * scale;
            double pz = offsetZ + wp.getPosition().getZ() * scale;
