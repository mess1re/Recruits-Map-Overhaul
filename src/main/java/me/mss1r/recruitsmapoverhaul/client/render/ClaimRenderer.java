...
public class ClaimRenderer {
    ...
    private static final int FOG_FILL_COLOR    = 0x180B1017;
    private static final int FOG_OUTLINE_COLOR = 0x703A4452;
    ...
    public static boolean isClaimExplored(RecruitsClaim claim) {
        for (ChunkPos chunk : claim.getClaimedChunks()) {
            if (isChunkVisibleToPlayer(chunk)) return true;
        }
        return false;
    }

    private static boolean isChunkLoadedOnClient(ChunkPos chunk) {
        net.minecraft.client.multiplayer.ClientLevel level = Minecraft.getInstance().level;
        return level != null && level.getChunkSource().getChunk(chunk.x, chunk.z, false) != null;
    }

    private static boolean isChunkVisibleToPlayer(ChunkPos chunk) {
        return !ClientManager.configFogOfWarEnabled
                || isAdminCreative()
                || ChunkTileManager.getInstance().isChunkExplored(chunk)
                || isChunkLoadedOnClient(chunk);
    }
    ...
    private static void renderClaimFill(GuiGraphics guiGraphics, RecruitsClaim claim, double offsetX, double offsetZ, double scale) {
        ...
        int factionFillColor = (88 << 24) | (getClaimColor(claim) & 0x00FFFFFF);

        for (ChunkPos chunk : claim.getClaimedChunks()) {
            boolean explored = isChunkVisibleToPlayer(chunk);
    ...
    private static void renderClaimPassiveOutline(GuiGraphics guiGraphics, RecruitsClaim claim, double offsetX, double offsetZ, double scale) {
        ...
        double thickness = Math.max(1.0, Math.min(3.0, scale * 0.5));
    ...
    private static void renderClaimSelectedOutline(GuiGraphics guiGraphics, RecruitsClaim claim, double offsetX, double offsetZ, double scale) {
        ...
        double borderThickness = Math.max(1.0, Math.min(4.0, scale));
    ...
    public static int getClaimColor(RecruitsClaim claim) {
        ...
        if (colorKey < 0 || colorKey >= FactionEditScreen.unitColors.size()) {
            return 0xFF888888;
        }
